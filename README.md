...
Windows 下基于 RePKG 的桌面图形界面，当前仓库已接入 PySide6 壳层，并在迁移期间保留原有 Tkinter 实现以继续浏览、筛选和提取本地已安装的 Wallpaper Engine 创意工坊壁纸资源。
...
`uv sync` / `pip install -r requirements.txt` 现在会一并安装 PySide6；旧版 Tk 界面仍然保留在仓库中，便于迁移期回归或对照。
...
其中 `test.py` 会覆盖共享服务，以及 PySide6 迁移后的 service / controller / worker 等无界面核心逻辑。

## 打包与发布

本仓库已提供 `uv + PyInstaller + GitHub Actions Release` 的发布链路，发布包现已切换为面向 `repkg_gui` PySide6 壳层构建，同时继续捆绑 `RePKG.exe` 和 `nekomusume.png`。
...
1. 安装 Python 和 `uv`
2. 同步依赖、编译检查 Python 源码，并验证 PySide6 壳层可导入
3. 运行现有测试
...
| `test.py` | `unittest` 测试入口，覆盖共享服务和 PySide6 迁移后的无界面核心逻辑 |
